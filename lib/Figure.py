...
def get_cdf(kvic_in, non_kvic_in, normalize = False):
    ...
    def find_next_ind(cur_ind, lst, i):
        while 1:
            if cur_ind + 1 == len(lst):
                break
            if lst[cur_ind + 1] > i:
                break
            cur_ind += 1
...
def get_cdf(kvic_in, non_kvic_in, normalize = False):
    ...
    kvic_interval = len(kvic_in) / gran
    x_kvic = [kvic_in[min(int(math.ceil(i * kvic_interval)), len(kvic_in) - 1)] for i in range(gran)]
    
    non_kvic_interval = len(non_kvic_in) / gran
    x_non_kvic = [non_kvic_in[min(int(math.ceil(i * non_kvic_interval)), len(non_kvic_in) - 1)] for i in range(gran)]
    
    
    y_kvic_count = [0] * gran
    y_non_kvic_count = [0] * gran
    
    # each in is already sorted
    def find_next_ind(cur_ind, lst, i):
        while 1:
            if cur_ind + 1 == len(lst):
                break
    ...
    ind = 0
    for i in kvic_in:
        ind = find_next_ind(ind, x_kvic, i)
        y_kvic_count[ind] += 1
     
    ind = 0
    for i in non_kvic_in:
        ind = find_next_ind(ind, x_non_kvic, i)
        y_non_kvic_count[ind] += 1
    ...
    return x_kvic, x_non_kvic, y_kvic, y_non_kvic
...
def make_plot_cdf(x_kvic, x_non_kvic, y_non_kvic, y_kvic, title, save_as, left):
    ...
    plt.semilogx(x_non_kvic,y_non_kvic,lw=5,marker='v',markersize=15,markevery=0.05,linestyle='--',label='All commits')  # 非 KVIC 图像
...
def get_cdf(kvic_in, non_kvic_in, normalize = False):
    ...
    ind = 0
    for i in kvic_in:
        ind = find_next_ind(ind, kvic_in, i)
        y_kvic_count[ind] += 1
