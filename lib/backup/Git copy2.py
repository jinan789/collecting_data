...
def get_modified_lines(commit, filter_empty_line = False, filter_comments = False):
    ...
    if filter_comments:
        #file_to_modified_lines_dict['drivers/gpu/drm/i915/gem/i915_gemfs.c'][8][2:].strip().startswith('*')
        a=1
...
def get_people_involved(commit):
    ...
    fixes_lst = [i for i in msg_lst if i.startswith("Fixes:")]
...
def get_commit_info(cur_commit):
    ...
    cur_commit_dict["num_signed_off"] = len(people_inv_dict["signed_off_lst"])
    cur_commit_dict["num_acked"] = len(people_inv_dict["acked_lst"])
    cur_commit_dict["num_cc"] = len(people_inv_dict["cc_lst"])
    cur_commit_dict["num_reviewed"] = len(people_inv_dict["reviewed_lst"])
    cur_commit_dict["num_tested"] = len(people_inv_dict["tested_lst"])
    cur_commit_dict["num_reported"] = len(people_inv_dict["reported_lst"])
    cur_commit_dict["num_suggested"] = len(people_inv_dict["suggested_lst"])
    cur_commit_dict["num_co_developed"] = len(people_inv_dict["co_developed_lst"])
